...
def run_template_matching(
    ...
    Args:
        ...
        min_contour_area: Minimum area in pixels to filter out noise contours.
    ...
    # Perform morphological opening/closing to remove small noise artifacts
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3))
    defect_mask_cleaned = cv2.morphologyEx(defect_mask, cv2.MORPH_OPEN, kernel)
    defect_mask_cleaned = cv2.morphologyEx(defect_mask_cleaned, cv2.MORPH_CLOSE, kernel)
    ...
    return diff_map, defect_mask_cleaned, bounding_boxes, processing_time_ms
